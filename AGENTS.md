...
- **PR merge strategy — HARD RULE, no exceptions:** Always rebase-merge. CLI: `gh pr merge <N> --rebase --repo atlashomesit/atlas-guest-portal`. Web UI: only the "Rebase and merge" button is available (repo settings enforce this since 2026-05-20). NEVER `--merge` or `--squash`. Full policy: [atlas-e2e/docs/governance/PR-MERGE-STRATEGY.md](../atlas-e2e/docs/governance/PR-MERGE-STRATEGY.md).
