...
- **PR merge strategy:** Default to "Create a merge commit" for `dev → main` PRs — `gh pr merge <N> --merge --repo atlashomesit/atlas-guest-portal`. "Rebase and merge" is allowed for short-lived `feature/* → dev` PRs. Squash is never allowed. Full policy + why we reverted the 2026-05-20 rebase-only rule: [atlas-e2e/docs/governance/PR-MERGE-STRATEGY.md](../atlas-e2e/docs/governance/PR-MERGE-STRATEGY.md).
