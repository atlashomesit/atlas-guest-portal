...
- **PRs and CI:** Run lint, build, and tests locally before opening a PR (see [CONTRIBUTING.md](CONTRIBUTING.md)) — there is no CI build/test gate on `dev` PRs to catch it for you. `.github/workflows/ci.yml` and its **build** required check were retired fleet-wide (TASK-101886, 2026-09-23; the unit bar it fed had already stopped gating anything since 2026-09-09). `dev` branch protection currently requires no status checks, so a PR merges once its author has verified it locally. The workflows that still run on a `dev` PR (`Author email allowlist`, `Allow feature → dev`) are branch/author policy checks, not build/test gates.
