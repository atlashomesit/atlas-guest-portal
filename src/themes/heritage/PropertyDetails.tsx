 ...
 *
 * Recovered composition (not stale logic): the JSX/CSS below is the pre-coral Atlas
 * property-details page as it stood at `d1ab2590^` (before the "Sandstone Coral"
 * re-theme rewrote `Homepage_PropertyDetails.tsx`/`.css` in place — ADR-0081 amendment
 * 2026-07-17 pt.4). The diff between this file and the current `classic` page
 * (`src/components/homepage_components/homepage_Propertydetails/Homepage_PropertyDetails.tsx`)
 * is entirely presentational — hardcoded colors instead of the coral CSS-variable tokens,
 * gallery gradient fallbacks instead of the SBC-branded design-wash SVGs, no "A note from
 * your host" lavender callout panel (a coral-redesign addition) — no booking/payment/
 * listing-data logic was changed or forked; all imports below resolve to the CURRENT
 * shared library (`@/api`, `@/hooks`, `@/utils`, `@/tenant`, `@/components/...`), including
 * `propertyMapMode`'s map-selection logic, which is imported from its original shared
...
const GuestAssistant = lazy(() => import('@/components/GuestAssistant')); // TASK-1728
const VirtualTourSection = lazy(() => import('@/components/VirtualTourSection')); // Task 37
const ReviewSummary = lazy(() => import('@/components/ReviewSummary')); // TASK-4404
...
// TASK-7012 (founder ruling, 2026-08-03): mirrors Homepage_PropertyDetails.tsx — this panel
// deliberately says NOTHING about the universal grace window and takes no `graceHours`. It renders
// with no selected check-in date, so there is nothing to void against, and atlas-api VOIDS the grace
// window when check-in falls inside it from booking time
// (`CancellationPolicyWindow.ComputeEffectiveFreeCancellationDeadlineUtc`): a same-day or next-day
// booker gets NO grace refund. A listing-level `graceHours` means the FEATURE is enabled for that
// listing, not that this guest's dates qualify. The grace window is disclosed ONLY by the booking
// widget's grace strip, which gates on `resolveApplicableGraceHours` once real dates are picked.
            ...
            resolveListing(String(listingIdParam), controller.signal)
                .then(async (apiListing: ListingDetail | null) => {
                    ...
                    const mapped: Property = {
                        ...
                        // TASK-8221: public listing DTO uses baseNightlyRate / propertyRating / reviewCount
  ...
  if (Number.isFinite(lid) && lid > 0) {
    ...
    return (
        ...
        {/* =====================================================================
           ...
           ===================================================================== */}
        <div className="pp-root" data-testid="heritage-property-details">
              ...
              <div>
                ...
                <section className="pp-section" style={{ paddingTop: 28 }} aria-label="About the host">
                  ...
                  {/* "What verified means" panel — TASK-5181: also the CROSS-LAYOUT canary
                      ...
                      fabricated host-note quote panel below. TASK-4987's E2E contrast gate needs
                      ONE always-rendered --lavender-soft/--lavender-text surface per layout to
                      corroborate the page actually painted before trusting a zero-violation axe
                      scan. That panel's content is now gated on a real per-listing field
                      (data.hostAbout) and correctly does not render when that field is empty, so
                      it can no longer anchor an unconditional canary. This panel is real,
                      always-rendered, non-fabricated platform copy — founder ruling 2026-07-30
                      re-points the gate here, resolving the TASK-4987 deadlock without depending
                      on content that may legitimately be absent. */}
                  <div
                ...
                {/* From your hosts — TASK-5181: renders ONLY when the listing has a real,
                    ...
                    rule shared by both layouts. Heading text is deliberately different from
                    Homepage_PropertyDetails.tsx's panel (TASK-4987 layout-differentiation). */}
                <HostAboutNote
                  hostAbout={ppHostAbout}
                  hasRealHost={ppHasRealHost}
                  hostDisplayName={ppHostDisplayName}
                  heading="From your hosts"
